...
# # Configurations
# VIDEO_PATH =  "data/YouTube.mp4" # Path to input video or webcam index (0)
# OUTPUT_PATH = 'outputs/output_video.mp4'  # Path to save the processed video
# YOLO_WEIGHTS = 'weights/yolov9-c.pt'  # Path to YOLO model weights
...
def main():
    ...
    frame_no = 0
    frame_skip = 6  # Define frame skip
    ...
    while True:
        ...
        if frame_no % frame_skip == 0:
            ...
            for det in detections:
                ...
                # Filter detections for "person" class (ID 0) and confidence threshold
                if class_id == 0 and confidence >= CONFIDENCE_THRESHOLD:
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), person_color, 2)
                    label = f"Person {confidence:.2f}"
                    cv2.putText(frame, label, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, person_color, 2)
                    
                    # Apply blur if enabled
                    if BLUR_PEOPLE:
                        frame[int(y1):int(y2), int(x1):int(x2)] = cv2.GaussianBlur(frame[int(y1):int(y2), int(x1):int(x2)], (99, 99), 30)
